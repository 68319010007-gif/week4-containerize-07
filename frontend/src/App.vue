...
const sortBy          = ref('name-asc') // การเรียงลำดับสินค้า
...
// กรองสินค้าตาม search + catFilter พร้อมกัน แล้วเรียงตาม sortBy
const filtered = computed(() => {
  ...
  const sorted = [...list]
  switch (sortBy.value) {
    case 'name-desc':  sorted.sort((a, b) => b.name.localeCompare(a.name, 'th')); break
    ...
    default:           sorted.sort((a, b) => a.name.localeCompare(b.name, 'th'))
  }
  return sorted
})
